...
public class EMethodMapManage {
	...
	private Object sendMethodMessages(EMethodMessage msg){
		if(msg.getName() == null || msg.getName().equals(""))
			return false;
		if(isMessageInfo && !msg.getName().equals("Log"))
			sendMethodMessage("Log",msg.getObject(), "Touch:"+msg.toString()+" ing....");
		EMethodMap m = _eventMap.get(msg.getName());
		if(m==null) return false;
		try {
			if(!msg.getName().equals("Log"))
				sendMethodMessage("Log",m.getObj(), "Method："+m.getMethod().getName()+",Touch : "+msg.getName()+" ok!");
			return m.invoke(msg);
		} catch (Exception e) {
			e.printStackTrace();
		}
